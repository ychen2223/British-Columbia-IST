...
ratio_control <- df_hold %>%
  ...
  # mutate(ratio = ifelse(v == 0 & area_of_residence != "Not elsewhere specified" & geo != "Not specified province", 0, ratio)) %>%
  group_by(area_of_residence, date) %>%
...
s_pro_1 <- df_hold %>%
  ...
  ungroup() %>%
  mutate(revised_s = case_when(area_of_residence == "Not elsewhere specified" | geo == "Not specified province"| date <= "2023-01-01" ~ revised_s,
...
#   #                              date >= "2024-01-01"~ revised_spend_per[date == "2023-01-01"] * growth)) %>%
#   # ungroup() %>%
#   # mutate(revised_s = case_when(area_of_residence == "Not elsewhere specified" | geo == "Not specified province" ~ revised_s,
#   #                              TRUE ~ v * revised_spend_per)) %>%
#   select(-c(s, ratio, growth)) %>%
#   mutate(revised_s = ifelse(is.na(revised_s), 0, revised_s)
#          # ,
...
```{r}
# need the s_forecast_3_pro_re calculated previously
test4 <- s_forecast_complete %>%
  rbind(s_forecast_3_pro) %>%
  filter(area_of_residence %in% c("Residents of countries other than the United States of America") & date <= "2023-01-01" & !(geo %in% c("Yukon", "Northwest Territories", "Nunavut"))) %>%
...
```{r}
provinces_3 <- s_forecast_complete %>%
...
s_forecast_complete2 <- s_forecast_complete %>%
  filter(!(geo %in% c("Yukon", "Northwest Territories", "Nunavut"))) %>%
  rbind(provinces_3)
```
...
```
```{r}
colSums(is.na(s_forecast_complete2))
```
```{r}
final_df_s <- s_forecast_complete2
```
