...
```{r}
check_s_ovs_1_v1 <- spend_pro_data3 %>%
  ...
  select(geo, date, s_forecast, ratio_s)

s_forecast_ovs_1_can <- spend_pro_data3 %>%
  filter(year <= "2023-01-01" & area_of_residence == "Residents of countries other than the United States of America" & geo != "Canada") %>%
  group_by(area_of_residence, year) %>%
  summarise(value_can = sum(value, na.rm = TRUE), .groups = "drop") %>%
  ungroup() %>%
  mutate(date = as.Date(paste0(year, "-01-01"))) %>%
  mutate(geo = "Canada")

check_s_ovs_1_v2 <- spend_pro_data3 %>%
  filter(year <= "2023-01-01" & area_of_residence == "Residents of countries other than the United States of America" & geo == "Canada") %>%
  group_by(year, geo, area_of_residence) %>%
  summarise(value = sum(value, na.rm = TRUE), .groups = "drop") %>%
  ungroup() %>%
  mutate(date = as.Date(paste0(year, "-01-01"))) %>%
  left_join(s_forecast_ovs_1_can, by = c("geo", "area_of_residence", "date")) %>%
  mutate(gap = value - value_can,
         gap = ifelse(gap < 0, 0, gap)) %>%
  mutate(geo = "Not specified province") %>%
  mutate(ratio_s = gap/0.000000000001) %>%
  select(geo, date, s_forecast = gap, ratio_s)

check_s_ovs_1 <- check_s_ovs_1_v1 %>%
  rbind(check_s_ovs_1_v2)

check_s_ovs_1_ratio <- check_s_ovs_1 %>%
  group_by(date) %>%
  mutate(s_can = sum(s_forecast, na.rm = TRUE),
         ratio = s_forecast/s_can) %>%
  ungroup()
  
check_s_ovs_1_v2_re <- check_s_ovs_1_v2 %>%
  mutate(area_of_residence = "Residents of countries other than the United States of America") %>%
  rename(value = s_forecast) %>%
  select(-ratio_s)
```
...
<!-- s_pro_2 <- s_pro_1 %>% -->
<!--   mutate(s_forecast = ifelse(geo == "Not specified province" | area_of_residence == "Not elsewhere specified", 0.000000000001, s_forecast)) %>% -->
...
<!--   mutate(date = as.Date(paste0(year, "-01-01"))) %>% -->
<!--   select(-c(year)) %>% -->
<!--   rbind(check_s_ovs_1_v2_re) %>% -->
<!--   left_join(s_forecast_ovs_2, by = c("geo", "area_of_residence", "date")) %>% -->
<!--   mutate(ratio_s = value/s_forecast, -->
<!--          ratio_s = ifelse(is.na(ratio_s) | is.infinite(ratio_s), 0, ratio_s)) -->
<!-- ``` -->
...
<!--   return(list(s_pro_3 = s_pro_3, -->
<!--               s_can_3 = s_can_3, -->
<!--               test_can_3 = test_can_3, -->
<!--               s_pro_3_final = s_pro_3_final, -->
<!--               s_forecast_ovs_3 = s_forecast_ovs_3,  -->
<!--               check_s_ovs_3 = check_s_ovs_3)) -->
<!-- } -->
...
```{r}
s_pro_3 <- results[[2000]]$s_pro_3
s_can_3 <- results[[2000]]$s_can_3
check_s_ovs_3 <- results[[2000]]$check_s_ovs_3
test_can_3 <- results[[2000]]$test_can_3
```
...
final_org_ctry_v3 <- final_org_ctry_v %>%
  filter(!(geo %in% c("Yukon", "Northwest Territories", "Nunavut"))) %>%
  rbind(final_org_ctry_v2)
```

```{r}
s_forecast2 <- df_hold2 %>%
  left_join(final_org_ctry_v3, by = c("geo", "area_of_residence" = "country_of_residence", "date")) %>%                          #visits
  left_join(select(s_forecast_complete, geo, area_of_residence, date, s_forecast, revised_spend_per), by = c("geo", "area_of_residence", "date")) %>%
  left_join(gts_spend_per_complete, by = c("area_of_residence" = "s_ttl", "date")) %>%
  mutate(v = ifelse(is.na(v), 0, v),
         revised_spend_per = ifelse(is.na(revised_spend_per) | is.infinite(revised_spend_per), 0, revised_spend_per)) %>%
  group_by(geo, area_of_residence) %>%
  mutate(revised_spend_per = case_when(date <= "2023-01-01" ~ revised_spend_per,
                               date >= "2024-01-01"~ revised_spend_per[date == "2023-01-01"] * growth)) %>%
  ungroup() %>%
  mutate(s_forecast = case_when(date <= "2023-01-01" ~ s_forecast,
...
s_forecast_complete2_tot <- s_forecast_complete2 %>%
  filter(area_of_residence == "United States of America residents, tourists (overnight)") %>%
  rbind(s_forecast_complete2_ovs) %>%
  group_by(geo, date) %>%
  summarise(v = sum(v, na.rm = TRUE),
            s_forecast = sum(s_forecast, na.rm = TRUE),
            revised_spend_per = s_forecast/v,
...
#gap for countries spending in Canada
```{r}
test6  <- s_forecast_complete3 %>%
  filter(date <= "2023-01-01" & geo == "Canada") %>%
  left_join(s_hist4, by = c("geo", "area_of_residence", "date")) %>%
  mutate(gap_s = value - s_forecast)
```

```{r}
s_forecast_complete4 <- s_forecast_complete3 %>%
  left_join(select(test5, geo, area_of_residence, date, gap), by = c("geo", "area_of_residence", "date")) %>%
  mutate(s_forecast = s_forecast + gap,
         revised_spend_per = s_forecast/v, 
         revised_spend_per = ifelse(is.na(revised_spend_per) | is.infinite(revised_spend_per), 0, revised_spend_per)) %>%
  select(-c(gap))
```
...
```{r}
colSums(is.na(s_forecast_complete3))
```
```{r}
