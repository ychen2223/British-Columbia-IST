...
list2 <- c("United States of America residents entering Canada", "Brazil", "Caribbean", "Mexico", "Belgium", "France", "Germany", "Italy", "Netherlands", "Switzerland", "United Kingdom", "Northern Africa", "Australia", "China", "Hong Kong", "India", "Japan", "New Zealand", "Antarctica and Adjacent Islands") # took out scandinavia and southern Asia
```
...
spend_can_ovs <- spend_pro_ovs %>%
  group_by(area_of_residence, date) %>%
  summarise(value = sum(value, na.rm = TRUE), .groups = "drop") %>%
  ungroup() %>%
  mutate(geo = "Canada", .before = area_of_residence)

ratio_can <- spend_pro_ovs %>%
  rbind(spend_can_ovs) %>%
  group_by(area_of_residence, date) %>%
  mutate(ratio = value/value[geo == "Canada"]) %>%
  ungroup()
```
...
#   left_join(select(s_hist4, date, area_of_residence, spend_per), by = c("area_of_residence", "date")) %>%                       #spending
#   # left_join(select(ratio_can, geo, date, ratio), by = c("geo", "date")) %>%                                                     #split total Canada
...
s_pro_1 <- df_hold %>%
  ...
  mutate(v = ifelse(is.na(v), 0, v),
         revised_s = case_when(area_of_residence == "United States of America residents, tourists (overnight)" ~s,
                               TRUE ~ s*ratio),
