...
#country and rest of region to Canada history
```{r}
s_hist1 <- spend_data3 %>%
...
#country and rest of region to Canada history, filter <= 2023
```{r}
s_hist2 <- s_hist1 %>%
...
```{r}
list1 <- c("United States of America residents entering Canada", "Brazil", "Caribbean", "Mexico", "Belgium", "France", "Germany", "Italy", "Netherlands", "Denmark", "Finland", "Iceland", "Norway", "Sweden", "Switzerland", "United Kingdom", "Northern Africa", "Australia", "China", "Hong Kong", "India", "Japan", "New Zealand", "Southern Asia", "Americas, countries other than the United States of America", "Europe", "Africa", "Asia", "Oceania")

list2 <- c("United States of America residents entering Canada", "Brazil", "Caribbean", "Mexico", "Belgium", "France", "Germany", "Italy", "Netherlands", "Switzerland", "United Kingdom", "Northern Africa", "Australia", "China", "Hong Kong", "India", "Japan", "New Zealand", "Southern Asia")
```
...
rest_as <- org_ctry %>%
  filter(country_of_residence %in% c("Australia", "China", "Hong Kong", "India", "Japan", "New Zealand", "Southern Asia")) %>%
  group_by(geo, date) %>%
  summarise(value_sum = sum(value, na.rm = TRUE), .groups = "drop") %>%
  ungroup() %>%
  left_join(org_ctry %>% 
...
final_org_ctry_v <- final_df_v %>%
  ...
  select(geo, country_of_residence, date, value) %>%
  rbind(scan, rest_ame, rest_eur, rest_afr, rest_as) %>%
  rename(v = value) %>%
...
```{r}
s_hist3 <- s_hist2 %>%
  left_join(final_org_ctry_v, by = c("geo", "area_of_residence" = "country_of_residence", "date")) %>%
...
test_gts <- data.frame(orig_code = unique(gts_spend_per$orig_code)) 
...
s_pro_1 <- df_hold %>%
  ...
  left_join(select(s_hist3, date, area_of_residence, spend_per), by = c("area_of_residence", "date")) %>%                       #spending
...
s_can_1 <- s_pro_1 %>%
  ...
  mutate(revised_spend_per = s_forecast/v, .before = s_forecast)
...
test2 <- s_can_1 %>%
  ...
  left_join(s_hist3, by = c("geo", "area_of_residence", "date")) %>%
  mutate(gap_s = s_forecast - value)  #gap is very small, except Africa
```

```{r}
reg_def2 <- data.frame(s_ttl= c("Americas, countries other than the United States of America - other countries", "Mexico", "Caribbean", "Brazil", "Europe - other countries", "Belgium", "France", "Germany", "Netherlands", "Switzerland", "Scandinavia", "United Kingdom", "Italy", "Africa - other countries", "Northern Africa", "Asia and Oceania - other countries", "China", "Hong Kong", "Japan", "Southern Asia", "India", "Australia", "New Zealand"),
...
#see if province matches total international history
```{r}
...
test3 <- s_forecast_complete %>%
  filter(area_of_residence %in% c("United States of America residents, tourists (overnight)", "Residents of countries other than the United States of America", "Non-resident visitors to Canada") & date <= "2023-01-01") %>%
